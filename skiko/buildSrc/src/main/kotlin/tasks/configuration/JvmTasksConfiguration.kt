...
fun SkikoProjectContext.createCompileJvmBindingsTask(
    ...
    if (JavaVersion.current() < JavaVersion.VERSION_17) {
        error(
            "JDK 17+ is required, but Gradle JVM is ${JavaVersion.current()}. " +
    ...
    val moduleBinaryInputs = binaryModule.resolveBinaryInputs(
        ...
        targetArch,
        TargetEnv.JVM,
        skiaDir.resolve("out/${buildType.id}-$target").absolutePath
    )
    ...
    flags.set(
        listOf(
            ...
            *osFlags,
            *moduleBinaryInputs.compileFlags.toTypedArray()
        )
    )
}
...
fun SkikoProjectContext.configureGenerateSymbolsList(
    ...
    project.tasks.register<GenerateSymbolsListTask>("generateSymbolsList$suffix") {
        ...
        project.skikoExtensionModules().forEach { module ->
            ...
            moduleObjectFiles.from(mCompileTask.map {
                it.outDir.get().asFile.walk().filter { it.name.endsWith(".o") || it.name.endsWith(".obj") }.toList()
            })
            ...
            if (targetOs.isMacOs) {
                ...
                dependsOn(mObjcTask)
                moduleObjectFiles.from(mObjcTask.map {
                    it.outDir.get().asFile.walk().filter { it.name.endsWith(".o") }.toList()
                })
            }
        }
        ...
        val coreBinaryInputs = skikoCoreModule.resolveBinaryInputs(targetOs, targetArch, TargetEnv.JVM, skiaBinDir)
        val extensionModuleBinaryInputs = project.skikoExtensionModules()
            .map { it.resolveBinaryInputs(targetOs, targetArch, TargetEnv.JVM, skiaBinDir) }

        skiaLibs.from(project.files(coreBinaryInputs.staticArchivePaths + coreBinaryInputs.directStaticArchivePaths))

        moduleLibs.from(project.files(extensionModuleBinaryInputs.flatMap { it.staticArchivePaths }))
    }
}
...
fun SkikoProjectContext.createLinkJvmBindings(
    ...
    val osFlags: Array<String>
    val binaryModule = project.currentSkikoExtensionModuleOrNull() ?: skikoCoreModule
    val moduleBinaryInputs = binaryModule.resolveBinaryInputs(targetOs, targetArch, TargetEnv.JVM, skiaBinDir)
    val isCore = binaryModule.kind == SkikoModuleKind.CORE
    val linksCore = binaryModule.dependsOnCore
    libFiles = project.files(moduleBinaryInputs.staticArchivePaths)
    ...
    when (targetOs) {
        OS.MacOS -> {
            ...
            val additionalFlags = if (linksCore) {
        ...
        OS.Linux -> {
            osFlags = mutableListOf<String>().apply {
                ...
                moduleBinaryInputs.dynamicLibNames.forEach { add("-l$it") }
                if (linksCore) {
                    // Cleaner dependency picking driven entirely by our module matrix definitions
                    addAll(moduleBinaryInputs.staticArchivePaths)
                    val coreLinkTaskName = "linkJvmBindings" + joinToTitleCamelCase(targetOs.id, targetArch.id)
                    val coreLinkTask = project.rootProject.tasks.named<LinkSkikoTask>(coreLinkTaskName)
                    dependsOn(coreLinkTask)
        ...
        OS.Android -> {
            ...
            moduleBinaryInputs.dynamicLibNames.forEach { androidFlags.add("-l$it") }
            if (linksCore) {
                val coreLinkTaskName = "linkJvmBindings" + joinToTitleCamelCase(targetOs.id, targetArch.id)
                val coreLinkTask = project.rootProject.tasks.named<LinkSkikoTask>(coreLinkTaskName)
                dependsOn(coreLinkTask)
    ...
    flags.addAll(moduleBinaryInputs.linkFlags)
            ...
            if (unexportedSymbols.exists()) {
                when (targetOs) {
                    ...
                    OS.Linux, OS.Android -> {
                        ...
                        result.addAll(moduleBinaryInputs.directStaticArchivePaths)
                    }
...
fun SkikoProjectContext.createDownloadCodeSignClientDarwinTask(
    targetOs: OS,
    hostArch: Arch
) = project.registerSkikoTask<de.undercouch.gradle.tasks.download.Download>(
    "downloadCodeSignClient",
    targetOs,
    hostArch
) {
...
fun SkikoProjectContext.setupJvmTestTask(
    ...
    val skikoAwtRuntimeJarForTests = createJvmJar(
        targetOs, targetArch, skikoAwtJarForTests, libBaseName = project.name,
    ...
    tasks.withType<Test>().configureEach {
        ...
        options {
            ...
            systemProperty(
                ...
                File(project.projectDir, "src/jvmTest/screenshots").absolutePath
            )
            systemProperty(
                "skiko.test.font.dir",
                File(project.projectDir, "src/commonTest/resources/fonts").absolutePath
            )
            ...
            systemProperty(
                "skiko.test.ui.enabled",
                System.getProperty("skiko.test.ui.enabled", canRunUiTests.toString())
            )
            systemProperty("skiko.test.ui.renderApi", System.getProperty("skiko.test.ui.renderApi", "all"))
            systemProperty(
                "skiko.test.ui.renderApi.ignoreAssertsFor",
                System.getProperty("skiko.test.ui.renderApi.ignoreAssertsFor", "OPENGL")
            )
