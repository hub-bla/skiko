...
private class GraphiteMetalContext(private val context: GraphiteContext, private val recorder: Recorder) : GpuContextBackend {
    ...
    override fun submit(surface: GpuSurface, syncCpu: Boolean) {
...
fun makeMetalContext(devicePtr: NativePointer, queuePtr: NativePointer): GpuContext =
    GpuContext(makeGraphiteMetalContextBackend(devicePtr, queuePtr))

@InternalSkikoApi
fun makeGraphiteMetalContextBackend(devicePtr: NativePointer, queuePtr: NativePointer): GpuContextBackend {
    val context = GraphiteContext.makeMetal(devicePtr, queuePtr)
    return try {
        GraphiteMetalContext(context, context.makeRecorder()).also {
