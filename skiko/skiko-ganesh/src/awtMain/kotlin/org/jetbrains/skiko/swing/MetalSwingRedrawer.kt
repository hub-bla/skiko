...
import org.jetbrains.skia.flushAndSubmit
...
internal class MetalSwingRedrawer(
...
) : CoreSwingRedrawerBase(swingLayerProperties, analytics, GraphicsApi.METAL) {
    companion object {
        init {
            GaneshLibrary.load()
        }
    ...
    init {
        onContextInit()
        val limit = swingLayerProperties.gpuResourceCacheLimit
        if (limit >= 0) {
