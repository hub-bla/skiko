...
val skikoGaneshModule = project.currentSkikoExtensionModule()

val skikoGaneshProjectContext = SkikoProjectContext(
    project = project,
    skiko = skiko,
    kotlin = kotlin,
    ...
    currentExtensionModule = skikoGaneshModule
)

fun configureGaneshNativeTarget(os: OS, arch: Arch, target: KotlinNativeTarget) {
    skikoGaneshProjectContext.configureNativeBridgesForExtension(
        skikoGaneshModule,
        os,
        arch,
        target
    ) { skiaDir, targetString, currentBuildType ->
        skikoGaneshModule.staticLibraryPaths(
            skiaDir = skiaDir,
...
kotlin {
    ...
    skikoGaneshProjectContext.declareSkiaTasks()
    ...
    if (supportWeb) {
        val skiaWasmDir = skikoGaneshProjectContext.registerOrGetSkiaDirProvider(OS.Wasm, Arch.Wasm, false)

        skikoGaneshProjectContext.declareWasmTasks(
            isSideModule = true,
    ...
    if (supportNativeMac) {
        configureGaneshNativeTarget(OS.MacOS, Arch.X64, macosX64())
        configureGaneshNativeTarget(OS.MacOS, Arch.Arm64, macosArm64())
    }

    if (supportNativeLinux) {
        configureGaneshNativeTarget(OS.Linux, Arch.X64, linuxX64())
        configureGaneshNativeTarget(OS.Linux, Arch.Arm64, linuxArm64())
    }

    if (supportNativeIosArm64) {
        configureGaneshNativeTarget(OS.IOS, Arch.Arm64, iosArm64())
    }

    if (supportNativeIosSimulatorArm64) {
        configureGaneshNativeTarget(OS.IOS, Arch.Arm64, iosSimulatorArm64())
    }

    if (supportNativeIosX64) {
        configureGaneshNativeTarget(OS.IOS, Arch.X64, iosX64())
    }

    if (supportNativeTvosArm64) {
        configureGaneshNativeTarget(OS.TVOS, Arch.Arm64, tvosArm64())
    }

    if (supportNativeTvosSimulatorArm64) {
        configureGaneshNativeTarget(OS.TVOS, Arch.Arm64, tvosSimulatorArm64())
    }

    if (supportNativeTvosX64) {
        configureGaneshNativeTarget(OS.TVOS, Arch.X64, tvosX64())
    }
    ...
    sourceSets.commonMain.dependencies {
        ...
        as a transitive dependency of skiko-ganesh, and it will NOT appear in the published POM
        consumers MUST explicitly depend on both:
            - implementation("org.jetbrains.skiko:skiko-x")
            - implementation("org.jetbrains.skiko:skiko-ganesh-x")
         */
        compileOnly(project(":"))
    }
    ...
    skikoGaneshProjectContext.jvmMainSourceSet?.dependencies {
        implementation(kotlin("stdlib"))
    }

    skikoGaneshProjectContext.jvmTestSourceSet?.dependencies {
        implementation(libs.coroutines.test)
        implementation(kotlin("test-junit"))
        implementation(kotlin("test"))
    }
    skikoGaneshProjectContext.awtTestSourceSet?.dependencies {
        implementation(libs.kotlinx.benchmark.runtime)
    }
    skikoGaneshProjectContext.webMainSourceSet?.dependencies {
        implementation(libs.kotlinx.browser)
    }

    skikoGaneshProjectContext.awtMainSourceSet?.dependencies {
        implementation(libs.jetbrainsRuntime.api)
        implementation(libs.coroutines.core)
    }

    skikoGaneshProjectContext.androidMainSourceSet?.dependencies {
        implementation(libs.coroutines.android)
    }

    skikoGaneshProjectContext.wasmJsTest?.dependencies {
        implementation(kotlin("test-wasm-js"))
    }
    skikoGaneshProjectContext.webTestSourceSet?.dependencies {
        implementation(libs.coroutines.core)
    }

    skikoGaneshProjectContext.webTestSourceSet?.apply {
        resources.srcDirs(
...
if (supportAndroid) {
    ...
    configure<LibraryExtension> {
        compileSdk = 33
        namespace = "org.jetbrains.skia.ganesh"
    ...
    val os = OS.Android
    val skikoGaneshAndroidJar by project.tasks.registering(Jar::class) {
    ...
    for (arch in arrayOf(Arch.X64, Arch.Arm64)) {
        skikoGaneshProjectContext.createJvmJar(os, arch, skikoGaneshAndroidJar,
            libBaseName = skikoGaneshModule.libBaseName,
            includeIcu = false)
    }
    tasks.matching { name == "publishAndroidReleasePublicationToMavenLocal" }.configureEach {
        // It needs to be compatible with Gradle 8.1
        dependsOn(skikoGaneshAndroidJar)
    }
...
if (supportAwt) {
    ...
    val skikoGaneshAwtJarForTests by project.tasks.registering(Jar::class) {
    ...
    skikoGaneshProjectContext.setupJvmTestTask(skikoGaneshAwtJarForTests, targetOs, targetArch, extraRuntimeJars=listOf(rootRuntimeJar))
}
...
skikoGaneshProjectContext.declarePublications("ganesh")
